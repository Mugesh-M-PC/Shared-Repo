...
async function fillDoorLocked(formPage, crm, downloadedMedia) {
  ...
  // Block 1
  await safeFill(formPage, map.personMet, crm.tpcName);
  await safeFill(formPage, map.relation, crm.tpcIs || 'Neighbour');
  await safeFill(formPage, map.dependents, '0');
  await safeFill(formPage, map.noOfFamilyMembers, '0');
  await safeClick(formPage, map.spouseWorkingNo);
  await safeFill(formPage, map.noOfEarningMembers, '0');
  await safeFill(formPage, map.spouseWorkingDesc, 'NA');
  await safeFill(formPage, map.yearsCity, '0');
  await safeFill(formPage, map.yearsResidence, '0');
  ...
  // Block 5
  const easeOfLocateValue = getEaseOfLocateValue(crm.traceability || 'Easy');
  await safeCheck(
    formPage,
  ...
  await safeFill(formPage, map.negativeFeedback, crm.negativeCaseReason || comments);
  ...
  await safeSelectByValue(formPage, map.cpvRejectedReasons, !isCpvPositive && '08');
