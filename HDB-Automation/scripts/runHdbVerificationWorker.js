...
function getMemoryUsageSnapshot() {
  ...
  const toMegabytes = value => Math.round(value / 1024 / 1024);
...
function startMemoryLogger(logger = console) {
  const intervalMs = nonNegativeNumber(
    'HDB_MEMORY_LOG_INTERVAL_MS',
    5 * 60 * 1_000
  );
  const logMemoryUsage = () => {
    logger.log('HDB worker memory usage:', getMemoryUsageSnapshot());
  };
...
function createAutomationStatusReporter(request, options = {}) {
  const logger = options.logger || console;
  const clientId = options.clientId || process.env.CRM_CLIENT_ID;
  const baseUrl = options.baseUrl || process.env.CRM_BASE_URL;
  const heartbeatIntervalMs = Number(
    options.heartbeatIntervalMs ??
    process.env.HDB_AUTOMATION_STATUS_INTERVAL_MS ??
    60_000
  );
  ...
  if (
    ...
    throw new Error(
      'HDB_AUTOMATION_STATUS_INTERVAL_MS must be a positive number.'
    );
  }
  ...
  return async (status, reason = '') => {
    ...
    if (!['0', '1'].includes(normalizedStatus)) {
      throw new Error('Automation client status must be 0 or 1.');
    }

    const shouldSendHeartbeat =
      now() - lastReportedAt >= heartbeatIntervalMs;
    ...
    try {
      await updateAutomationClientStatus(request, normalizedStatus, {
        baseUrl,
        clientId,
      });
      lastReportedStatus = normalizedStatus;
      lastReportedAt = now();
      logger.log(
        `CRM automation client status updated to ${normalizedStatus}` +
        `${reason ? `: ${reason}` : '.'}`
      );
      return true;
    } catch (error) {
      logger.error(
        `CRM automation client status update to ${normalizedStatus} failed` +
...
async function runHdbVerificationWorker(argv = process.argv.slice(2)) {
  ...
  assertHdbEnvironment(process.env);
  ...
  let browser;
  let worker;
  let reportAutomationStatus = async () => false;
  let stopMemoryLogging = () => {};
  ...
  try {
    ...
    stopMemoryLogging = startMemoryLogger(logger);
    reportAutomationStatus = createAutomationStatusReporter(
    ...
    await reportAutomationStatus(0, 'worker starting');
    ...
    const sessionManager =
      new PortalSessionManager({
        ...
        onAuthAlert: async (context = {}) => {
          const alertPage =
            context.loginPage || loginPage;

          await showVisibleAuthAlert(alertPage).catch(error => {
            logger.error(
        ...
        onAuthenticationRequired: async () => {
          await reportAutomationStatus(
            0,
            'manual HDB login is required'
          );
        },
        onAuthenticated: async (context = {}) => {
          await reportAutomationStatus(
            1,
            context.stage === 'manual-login-completed'
              ? 'manual HDB login completed after OTP'
              : 'HDB listing session is ready'
          );
        },
      });
    ...
    try {
      ...
      await reportAutomationStatus(
        0,
        'worker stopped because of an error'
      );
      throw error;
    }
  } finally {
...
module.exports = {
  ...
  getHelpText,
  createAutomationStatusReporter,
