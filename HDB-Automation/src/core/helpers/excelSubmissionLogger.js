...
function isXlsxReportEnabled() {
  const configuredValue = String(
    process.env.HDB_XLSX_REPORT_ENABLED ?? 'false'
  ).trim().toLowerCase();
...
function initializeSubmissionReport(options = {}) {
  ...
  const xlsxEnabled = isXlsxReportEnabled();
  ...
  const operation = writeQueue.then(async () => {
    ...
    if (!xlsxEnabled) {
      ...
      if (csvFlushResult.storedInCsv) {
        cancelPendingCsvFlush(csvFlushResult.csvPath);
      } else {
      ...
      return {
        reportPath,
        csvPath: csvFlushResult.csvPath,
        pendingPath: getPendingPath(reportPath),
        xlsxEnabled: false,
        workbookSkipped: true,
        storedInWorkbook: false,
        pendingCount: 0,
        flushedCount: 0,
        reportReconciled: false,
