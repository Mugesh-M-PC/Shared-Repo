...
const DEFAULT_VALUES = Object.freeze({
  ...
  easeOfLocate: '02',
  landmark: 'NA',
  typeOfResidence: '07', // Flat / apartment
  ...
  cpvRejectedReasons: '08',
  remarks: 'Door Locked',
  noOfAttempts: '1',
  verificationAgent: '5003006', // BANRAD FINSERVE
  finalResult: '2', // Negative
});
...
async function fillDoorLocked(formPage, crm, manualAttachments) {
  ...
  const comments = baseComments.toLowerCase().includes('door locked')
    ? baseComments
    : baseComments
      ? `${baseComments}, Door Locked`
      : 'Door Locked';

  const negativeFeedback =
    crm.negativeCaseReason || DEFAULT_VALUES.negativeFeedback;
  ...
  await safeFill(
    formPage,
    map.relation,
    sanitizeStringOnly(crm.tpcIs, DEFAULT_VALUES.relation)
  );
  await safeFill(
    formPage,
    map.dependents,
    sanitizeNumericOnly(DEFAULT_VALUES.dependents)
  );
  await safeFill(
    formPage,
    map.noOfFamilyMembers,
    sanitizeNumericOnly(DEFAULT_VALUES.familyMembers)
  );
  await safeClick(
    formPage,
  ...
  await safeFill(
    ...
    sanitizeNumericOnly(DEFAULT_VALUES.earningMembers)
  );
  await safeFill(
    formPage,
  ...
  await safeFill(
    formPage,
    map.yearsCity,
    sanitizeNumericOnly(DEFAULT_VALUES.yearsInCity)
  );
  await safeFill(
    formPage,
    map.yearsResidence,
    sanitizeNumericOnly(DEFAULT_VALUES.yearsAtResidence)
  );
  const addressConfirmed = String(crm.doorNo || '').trim()
    ? getAddressConfirmBoolean(crm.doorNo)
    : DEFAULT_VALUES.addressConfirmed;
  await safeClick(
    formPage,
  ...
  await safeFill(
    formPage,
    map.latitude,
    sanitizeNumericOnly(crm.latitude, DEFAULT_VALUES.latitude, {
      allowDecimal: true,
      allowNegative: true,
    })
  );
  await safeFill(
    formPage,
    map.longitude,
    sanitizeNumericOnly(crm.longitude, DEFAULT_VALUES.longitude, {
      allowDecimal: true,
      allowNegative: true,
    })
  );
  ...
  await safeFill(
    ...
    sanitizeStringOnly(
      crm.tpcName,
      DEFAULT_VALUES.contactPerson
    )
  );
  await safeFill(
    formPage,
    map.telephoneNumber,
    sanitizeNumericOnly(crm.phone)
  );
  ...
  await safeFill(
    ...
    sanitizeStringOnly(
      crm.tpcName,
      DEFAULT_VALUES.nameOfPerson
    )
  );
  const doesApplicantStayHere = getYesNoBoolean(
    crm.doesApplicantStayHere || crm.applicantNameConfirm,
  ...
  await safeFill(
    ...
    sanitizeStringOnly(DEFAULT_VALUES.verifierComments)
  );
  if (!isCpvPositive) {
