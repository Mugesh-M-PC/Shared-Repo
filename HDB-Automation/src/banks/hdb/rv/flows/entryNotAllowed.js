...
async function fillEntryNotAllowed(formPage, crm, manualAttachments) {
  const comments = getRequiredVerifierComments(crm, DEFAULT_VALUES.verifierComments);
  ...
  await safeFill(
    formPage,
    map.personMet,
    sanitizeStringOnly(crm.tpcName, DEFAULT_VALUES.personMet)
  );
  await safeFill(
    formPage,
    map.relation,
  ...
  await safeFill(
    formPage,
    map.landmark,
    sanitizeStringOnly(getLandmarkValue(crm.landmark), DEFAULT_VALUES.landmark)
  );
